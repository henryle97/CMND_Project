...
parser.add_argument('--num_line', default=5, type=int, help='num line')
parser.add_argument('--reset_line_dir', default=True, type=str2bool, help='Delete old line detected folder ')
parser.add_argument('--single_image_test', default=False, type=str2bool, help='Test on single image or folder')
...
def test_net(net, image, text_threshold, link_threshold, low_text, cuda, poly):
    ...
    lines = cmnd_utils.detectLine(boxes)
    ...
    if args.show_time : print("2.Time detect text : {:.5f}s  ".format(t1-t0))

    return boxes, polys, lines
...
if __name__ == '__main__':
    ...
    t = time.time()
    if args.single_image_test == False:
        ...
        for k, image_path in enumerate(image_list):
            ...
            bboxes, polys, score_text,lines = test_net(net, image, args.text_threshold, args.link_threshold, args.low_text, args.cuda, args.poly)
            ...
            if lines is not None:
                cmnd_utils.cropLine(image_path, image[:,:,::-1],lines,num_line=args.num_line)
